...
- **Raw Markdown view** (`</>` toolbar button): switch the note to a plain-text Markdown editor where you can drag-select across lines, `Ctrl/Cmd+A`, copy, and edit freely. `Ctrl/Cmd+A` from the rendered view drops you here with everything selected. Toggle back to the rendered view with the same button. (The rendered view uses one editor per line, so cross-line selection only works in raw mode.)
